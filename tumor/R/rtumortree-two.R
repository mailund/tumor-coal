...
#' @param tau     Offset in time of the sampling of the two samples.
#' 
#' @export
rtumortree.two <- function(n1, n2, delta, gamma1, gamma2, tau) {
  ...
  gamma <- gamma1 + gamma2 # FIXME
  TA <- twoTumors_CaseB_TA(n1, n2, delta, gamma, tau)
  ...
  # Merge the two trees at time T_A
  tree <- bind(tree1, tree2, where = TA) # FIXME: WHERE ISN'T CORRECT
